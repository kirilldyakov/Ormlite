...
public class Graph_Energy extends Activity {
    ...
    void init() {
        ...
        data = null;
        txtGraphLabel.setText(getText(R.string.Graph_Energy_Name));
        if (calcEnergy())
            drawLineGraph(Color.RED);
        else
    ...
    public boolean calcEnergy() {
        ...
        for (int i = 0; i < data.length; i++) {
            double x = data[i].getX();//Неочень то хорошо нужна Date()
